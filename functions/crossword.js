...
function Crossword(words_in, clues_in, max_tries) {
  var GRID_ROWS = 50;
  var GRID_COLS = 50;
  ...
  function getSquareGrid(max_tries) {
    var best_grid = null;
    var best_ratio = 0;
    for (var i = 0; i < max_tries; i++) {
      var a_grid = this.getGrid(1);
      if (a_grid == null) continue;
      var ratio = Math.min(a_grid.length, a_grid[0].length) * 1.0 /
      ...
      for (var g = 0; g < groups.length; g++) {
        ...
        for (var i = 0; i < groups[g].length; i++) {
          ...
          if (!best_position) {
            ...
            var r = best_position['row'], c = best_position['col'],
      ...
      for (var c = 0; c < grid[r].length; c++) {
        ...
        for (var k in groups) {
          ...
          if (cell && cell[k] && cell[k]['is_start_of_word']) {
            ...
            groups[k].push({
              'position': position, 'index': index, 'clue': clues_in[index], 'word': words_in[index]
            });
  ...
  var canPlaceWordAt = function (word, row, col, direction) {
    ...
    if (direction == 'across') {
      ...
      if (col + word.length < grid[row].length && grid[row][col + word.length] !=
        null) {
        return false;
      }
    ...
    for (var i = 0; i < word.length; i++) {
      ...
      for (var j = 0; j < possible_locations_on_grid.length; j++) {
        ...
        if (intersections_across !== false) {
          bests.push({
            'intersections': intersections_across, 'row': r, 'col': c - i, 'direction': 'across'
          });
        }
        if (intersections_down !== false) {
          bests.push(
            { 'intersections': intersections_down, 'row': r - i, 'col': c, 'direction': 'down' });
        }
      }
    }
  ...
  return {
    grid: getGrid(max_tries),
    unplaced: getBadWords(),
  }
}
